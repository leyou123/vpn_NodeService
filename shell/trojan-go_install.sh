...
tls_generate() {
  if [[ -f "/root/ssl/fullchain.crt" ]] && [[ -f "/root/ssl/private.key" ]]; then
    ...
    if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force; then
        ...
        sleep 2
        [[ ! -d "/root" ]] && mkdir /root
        [[ ! -d "/root/ssl" ]] && mkdir "/root/ssl"
        if "$HOME"/.acme.sh/acme.sh --installcert -d "${domain}" --fullchainpath /root/ssl/fullchain.crt --keypath /root/ssl/private.key --ecc --force; then
            echo -e "${Info}证书配置成功 "
            sleep 2
        fi
    else
...
web_install() {
  [[ ! -d "${web_dir}" ]] && mkdir "${web_dir}"
  wget -O ${web_dir}/web.zip --no-check-certificate https://github.com/V2RaySSR/Trojan/raw/master/web.zip
  unzip -o -d ${web_dir} ${web_dir}/web.zip
}
